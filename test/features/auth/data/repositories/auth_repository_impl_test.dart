  ...
  group('AuthRepositoryImpl -', () {
    group('login', () {
      test('should return AuthSessionEntity when login succeeds', () async {
        ...
        when(
          () => mockSecureStorage.storeAccessToken(any()),
        ).thenAnswer((_) async {});
        when(
        ...
        verify(
          () => mockSecureStorage.storeAccessToken('test_access_token'),
        ).called(1);
        verify(
          () => mockSecureStorage.storeRefreshToken('test_refresh_token'),
        ).called(1);
      });
    ...
    group('logout', () {
      test('should return true when logout succeeds', () async {
        ...
        when(
          () => mockSecureStorage.deleteAccessToken(),
        ).thenAnswer((_) async {});
        when(
    ...
    group('refreshToken', () {
      test(
        'should return new AuthSessionEntity when token refresh succeeds',
        () async {
          // arrange
    ...
    group('updateProfile', () {
      test('should return updated UserEntity when update succeeds', () async {
        // arrange
        final updatedUser = testUser.copyWith(profile: testProfile);
        when(
