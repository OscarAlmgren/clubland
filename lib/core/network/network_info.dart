  ...
  Future<NetworkSpeed> estimateNetworkSpeed() async {
    ...
    switch (connectionType) {
      ...
      case ConnectionType.mobile:
        return NetworkSpeed.medium; // This could be enhanced with more detailed mobile network info
...
class NetworkDetails {
  ...
  @override
  String toString() {
    return 'NetworkDetails(isConnected: $isConnected, connectionType: $connectionType, isMetered: $isMetered, estimatedSpeed: $estimatedSpeed)';
  }
}
...
class NetworkMonitor {
  ...
  final StreamController<NetworkDetails> _networkController = StreamController<NetworkDetails>.broadcast();
    ...
    _connectivitySubscription = _networkInfo.onConnectivityChanged.listen(
      (isConnected) async {
        if (_networkInfo is EnhancedNetworkInfo) {
          final details = await (_networkInfo as EnhancedNetworkInfo).getNetworkDetails();
          ...
          _networkController.add(
            NetworkDetails(
              ...
              estimatedSpeed: isConnected ? NetworkSpeed.medium : NetworkSpeed.none,
            ),
          );
        }
      },
